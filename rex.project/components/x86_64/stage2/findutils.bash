...
mode_install_temp() {
	...
	logprint "Clean-up actions..."
	logprint "moving files to their proper locations..."
	# this looks wrong compared to ubuntu 18.04
	mv -v ${T_SYSROOT}/usr/bin/find ${T_SYSROOT}/bin
	assert_zero $?
	
	# also looks wrong and arbitrary
	sed -i 's|find:=${BINDIR}|find:=/bin|' ${T_SYSROOT}/usr/bin/updatedb
	assert_zero $?
