...
mode_install_temp() {
	...
	mv -v ${T_SYSROOT}/usr/bin/{cat,chgrp,chmod,chown,cp,date,dd,df,echo} ${T_SYSROOT}/bin
	assert_zero $?
	mv -v ${T_SYSROOT}/usr/bin/{false,ln,ls,mkdir,mknod,mv,pwd,rm} ${T_SYSROOT}/bin
	assert_zero $?
	mv -v ${T_SYSROOT}/usr/bin/{rmdir,stty,sync,true,uname} ${T_SYSROOT}/bin
	assert_zero $?
	mv -v ${T_SYSROOT}/usr/bin/{head,nice,sleep,touch} ${T_SYSROOT}/bin
	assert_zero $?
